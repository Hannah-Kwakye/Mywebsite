{{-- @include('instafeed') --}}
<div class="insta-feed ptb-120  gray-bg">
    <div class="pl-185 pr-185">
        <div class="section-title-2 text-center mb-50">
            <h2>Insta Feed</h2>
            <h4>Follow us on intagram. <span>@Ezonepro</span></h4>
        </div>
        <div class="instafeed-wrapper">
            <div class="instafeed-active owl-carousel">
                <div class="instafeed-img">
                    <img src="{{asset('img/instra/1.jpg')}}" alt="">
                </div>
                <div class="instafeed-img">
                    <img src="{{asset('img/instra/2.jpg')}}" alt="">
                </div>
                <div class="instafeed-img">
                    <img src="{{asset('img/instra/3.jpg')}}" alt="">
                </div>
                <div class="instafeed-img">
                    <img src="{{asset('img/instra/4.jpg')}}" alt="">
                </div>
                <div class="instafeed-img">
                    <img src="{{asset('img/instra/5.jpg')}}" alt="">
                </div>
                <div class="instafeed-img">
                    <img src="{{asset('img/instra/2.jpg')}}" alt="">
                </div>
            </div>
        </div>
    </div>
</div>
<!-- insta feed end -->